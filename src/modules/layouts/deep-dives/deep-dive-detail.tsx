...
import type { HowItWorksArticle } from "@/data/how-it-works";
...
type Props = {
  article: HowItWorksArticle;
  content: ReactNode;
  toc: TocEntry[];
  relatedTopics: RelatedTopic[];
  otherArticles: { slug: string; title: string; tagline: string; category: string }[];
};

export default function DeepDiveDetail({
  article,
  content,
  toc,
  relatedTopics,
          ...
          <article className="min-w-0 pb-4">
            <header className="border-b border-black/15 pb-9">
              <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-black/45">
                <Link href="/deep-dives" className="text-[#ff4d00] hover:underline">
                  Deep dives
                </Link>
                <span>·</span>
              ...
              <p className="mt-5 text-lg text-[#ff4d00]">{article.tagline}</p>
            ...
            <div className="article-copy">
                ...
                <section id="related" className="scroll-mt-28 mt-16">
                  <div className="flex items-baseline justify-between gap-4">
                    <div>
                      ...
                      <h2>The curriculum version of this.</h2>
                    </div>
                  </div>
              ...
              {otherArticles.length > 0 && (
                <section className="mt-16">
                  <p className="article-kicker">Keep reading</p>
                  <h2>More deep dives.</h2>
                  <div className="mt-5 grid gap-4 sm:grid-cols-2">
                    {otherArticles.map((other) => (
                      <Link
                        key={other.slug}
                        href={`/deep-dives/${other.slug}`}
